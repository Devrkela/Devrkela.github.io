for(let button of document.querySelectorAll("button")){
  const pressed_element = button.querySelector(".pressed");
  const dimension = button.querySelector(".decoration").getBoundingClientRect();

  pressed_element.style.width = 2 * dimension.width + dimension.height;
  pressed_element.style.height = 2 * dimension.width + dimension.height;      

  button.addEventListener("pointermove", function move(event){
