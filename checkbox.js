for(let checkbox of document.querySelectorAll(".checkbox")){
  const pressed_element = checkbox.querySelector(".pressed");
  const dimension = checkbox.querySelector(".decoration").getBoundingClientRect();
  ...
  checkbox.addEventListener("pointerdown" , function pressed(event){
...
for(let checkbox of document.querySelectorAll(".checkbox")){
  checkbox.addEventListener("pointerdown", function(){
    if(this.hasAttribute("aria-checked")){
      if(this.getAttribute("aria-checked") === "true") return this.setAttribute("aria-checked", "false");
